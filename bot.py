...
async def preguntar_ia(prompt: str, user_id: int, canal_id: int) -> str:
    try:
        ...
        contenido = [
            types.Content(role="model" if r == "assistant" else "user", parts=[types.Part(text=c)])
            for r, c in historial
        ] + [types.Content(role="user", parts=[types.Part(text=prompt)])]

        chat = gemini_client.models.generate_content(
            model=MODELO,
            contents=contenido,
        ...
        respuesta = ""
        if chat.candidates and chat.candidates[0].content and chat.candidates[0].content.parts:
            respuesta = (chat.candidates[0].content.parts[0].text or "").strip()
