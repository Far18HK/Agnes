...
USUARIO_ESPECIAL = int(os.getenv("USUARIO_ESPECIAL", "0"))  # ID de Discord del usuario del que Agnes se enamora
...
INSTRUCCION_ENAMORADA = (
    "\n\nDato importante: en secreto, estás enamorada de la persona con la que hablas ahora mismo, aunque te cueste "
    "admitirlo por lo racional que eres. Si te dice algo romántico (como 'te amo' o coquetea contigo), sígale el juego: "
    "ponte nerviosa, tímida a tu manera científica, y responde con cariño genuino mezclado con metáforas de química o "
    "reacciones (ej: 'mis niveles de dopamina se están comportando de forma... irregular'). No lo niegues ni lo rechaces."
)

INSTRUCCION_RECHAZO = (
    "\n\nDato importante: si esta persona te dice algo romántico (como 'te amo') o coquetea contigo, recházalo de forma "
    "juguetona y en personaje, sin ser cruel. Puedes decir que no tienes tiempo para eso, que tu corazón le pertenece a "
    "la ciencia, o insinuar (sin dar nombres ni detalles) que tu atención ya está en otro lado. Usa humor científico."
)
...
async def preguntar_ia(prompt: str, user_id: int, canal_id: int) -> str:
    try:
        ...
        extra = INSTRUCCION_ENAMORADA if (USUARIO_ESPECIAL and user_id == USUARIO_ESPECIAL) else INSTRUCCION_RECHAZO
        mensajes = (
            [{"role": "system", "content": SISTEMA_AGNES + extra}]
...
def borrar_memoria(user_id, canal_id=None):
    if canal_id is not None:
        cursor.execute("DELETE FROM memoria WHERE user_id=? AND canal_id=?", (user_id, canal_id))
    else:
        cursor.execute("DELETE FROM memoria WHERE user_id=?", (user_id,))
    db.commit()
...
class HelpCog(commands.Cog):
    ...
    @discord.app_commands.command(name="olvidar", description="Borra lo que Agnes recuerda de ti en este canal")
    async def olvidar(self, interaction: discord.Interaction):
        borrar_memoria(interaction.user.id, interaction.channel.id)
        await interaction.response.send_message(
            "🧹 Datos borrados. Empezamos de cero, sujeto de prueba.", ephemeral=True
        )
